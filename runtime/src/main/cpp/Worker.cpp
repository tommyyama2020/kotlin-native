...
enum { INVALID = 0, SCHEDULED = 1, COMPUTED = 2, CANCELLED = 3, THROWN = 4 };

enum { CHECKED = 0, UNCHECKED = 1 };
...
class Worker {
public:
    Worker(KInt id, bool errorReporting, KRef customName, WorkerKind kind) : id_(id), kind_(kind), errorReporting_(errorReporting) {
...
class Locker {
public:
    explicit Locker(pthread_mutex_t* lock) : lock_(lock) { pthread_mutex_lock(lock_); }
...
public:
    ...
    OBJ_GETTER0(consumeResultUnlocked) {
        ...
        if (state_ == THROWN) ThrowIllegalStateException();
...
public:
    ...
    Future* addJobToWorkerUnlocked(KInt id, KNativePtr jobFunction, KNativePtr jobArgument, bool toFront, KInt transferMode) {
...
void executeAfter(KInt id, KRef job, KLong afterMicroseconds) {
    if (!theState()->executeJobAfterInWorkerUnlocked(id, job, afterMicroseconds)) ThrowWorkerInvalidState();
}
...
KInt requestTermination(KInt id, KBoolean processScheduledJobs) {
    Future* future = theState()->addJobToWorkerUnlocked(id, nullptr, nullptr, /* toFront = */ !processScheduledJobs, UNCHECKED);
    if (future == nullptr) ThrowWorkerInvalidState();
    return future->id();
}
...
bool Worker::waitForQueueLocked(KLong timeoutMicroseconds, KLong* remaining) {
    while (queue_.size() == 0) {
        ...
        if (closestToRunMicroseconds == 0) {
            ...
            if (closestToRunMicroseconds > 10LL * 1000 * 1000 * 1000 * 1000) closestToRunMicroseconds = 10LL * 1000 * 1000 * 1000 * 1000;
    ...
    switch (job.kind) {
        ...
        case JOB_EXECUTE_AFTER: {
            ...
            try {
                ...
                if (errorReporting()) ReportUnhandledException(e.obj());
            }
            DisposeStablePointer(job.executeAfter.operation);
            break;
        }
...
void Kotlin_Worker_freezeInternal(KRef object) {
    if (object != nullptr) FreezeSubgraph(object);
}
