...
template <typename T> inline void copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count) {
    ...
    memmove(
        PrimitiveArrayAddressOfElementAt<T>(destinationArray, toIndex),
        PrimitiveArrayAddressOfElementAt<T>(array, fromIndex), count * sizeof(T));
}
...
OBJ_GETTER(Kotlin_CharArray_copyOf, KConstRef thiz, KInt newSize) {
    ...
    memcpy(
        PrimitiveArrayAddressOfElementAt<KChar>(result, 0), PrimitiveArrayAddressOfElementAt<KChar>(array, 0),
        toCopy * sizeof(KChar));
...
OBJ_GETTER(Kotlin_ImmutableBlob_toByteArray, KConstRef thiz, KInt startIndex, KInt endIndex) {
    ...
    memcpy(
        PrimitiveArrayAddressOfElementAt<KByte>(result, 0), PrimitiveArrayAddressOfElementAt<KByte>(array, startIndex),
        count);
    RETURN_OBJ(result->obj());
}
