...
public:
    ...
    static constexpr const char* indexToName[] = {"local ", "stack ", "perm  ", "frozen", "shared", "null  "};
    ...
    void incAlloc(size_t size, const ObjHeader* header) { objectAllocs[toIndex(header, 0)]++; }
    ...
    static double percents(uint64_t value, uint64_t all) { return all == 0 ? 0 : ((double)value / (double)all) * 100.0; }
    ...
    void printStatistic() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (i == 0 || i == 3 || i == 4) konan::consolePrintf("Object %s alloc: %lld\n", indexToName[i], objectAllocs[i]);
        }
        konan::consolePrintf("\n");
        ...
        for (int i = 0; i < 12; i++) {
            for (int j = 0; j < 12; j++) {
                ...
                if (i < 6 && j < 6) heapUpdateRefs += updateCounters[i][j];
                if (i >= 6 && j >= 6) stackUpdateRefs += updateCounters[i][j];
            }
        }
        konan::consolePrintf(
                "Total updates: %lld, stack: %lld(%.2lf%%), heap: %lld(%.2lf%%)\n", allUpdateRefs, stackUpdateRefs,
                percents(stackUpdateRefs, allUpdateRefs), heapUpdateRefs, percents(heapUpdateRefs, allUpdateRefs));
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 6; j++) {
                if (updateCounters[i][j] != 0)
                    konan::consolePrintf(
                            "UpdateHeapRef[%s -> %s]: %lld (%.2lf%% of all, %.2lf%% of heap)\n", indexToName[i], indexToName[j],
                            updateCounters[i][j], percents(updateCounters[i][j], allUpdateRefs),
        ...
        for (int i = 6; i < 12; i++) {
            for (int j = 6; j < 12; j++) {
                if (updateCounters[i][j] != 0)
                    konan::consolePrintf(
                            "UpdateStackRef[%s -> %s]: %lld (%.2lf%% of all, %.2lf%% of stack)\n", indexToName[i - 6], indexToName[j - 6],
                            updateCounters[i][j], percents(updateCounters[i][j], allUpdateRefs),
        ...
        int count = 0;
        for (auto it : keys) {
            konan::consolePrintf("%d bytes -> %d times  ", it, (*allocationHistogram)[it]);
            if (++count % perLine == (perLine - 1) || (count == keys.size())) konan::consolePrintf("\n");
        }
        ...
        konan::consolePrintf(
                ...
                percents(atomicReleaseRefs, allReleases), releaseCyclicRefs, percents(releaseCyclicRefs, allReleases));
    }
};
    ...
    void processAbandoned() {
        if (this->releaseList != nullptr) {
            ...
            if (hadNoStateInitialized) {
                ...
                if (atomicGet(&aliveMemoryStatesCount) == 0) return;

                memoryState = InitMemory(); // Required by ReleaseHeapRef.
            }

            processEnqueuedReleaseRefsWith([](ObjHeader* obj) { ReleaseHeapRef(obj); });
...
#define CONTAINER_DESTROY_STAT(state, container) state->statistic.incFree(container);
...
#define UPDATE_REF_STAT(state, oldRef, newRef, slot, stack) state->statistic.incUpdateRef(oldRef, newRef, stack);
#define UPDATE_ADDREF_STAT(state, obj, atomic, stack) state->statistic.incAddRef(obj, atomic, stack);
#define UPDATE_RELEASEREF_STAT(state, obj, atomic, cyclic, stack) state->statistic.incReleaseRef(obj, atomic, cyclic, stack);
...
#if TRACE_MEMORY
#define INIT_TRACE(state) memoryState->containers = konanConstructInstance<ContainerHeaderSet>();
...
#define DEINIT_TRACE(state)
#endif
#define CONTAINER_ALLOC_TRACE(state, size, container) MEMORY_LOG("Container alloc %d at %p\n", size, container)
#define CONTAINER_DESTROY_TRACE(state, container) MEMORY_LOG("Container destroy %p\n", container)
#define OBJECT_ALLOC_TRACE(state, size, object) MEMORY_LOG("Object alloc %d at %p\n", size, object)
...
protected:
    ...
    void SetHeader(ObjHeader* obj, const TypeInfo* type_info) {
        ...
        if ((type_info->flags_ & TF_IMMUTABLE) != 0) header_->refCount_ |= CONTAINER_TAG_FROZEN;
        if ((type_info->flags_ & TF_ACYCLIC) != 0) header_->setColorEvenIfGreen(CONTAINER_TAG_GC_GREEN);
    }
};
...
public:
    // Single instance.
    explicit ObjectContainer(MemoryState* state, const TypeInfo* type_info) { Init(state, type_info); }

    // Object container shalln't have any dtor, as it's being freed by
    // ::Release().

    ObjHeader* GetPlace() const { return reinterpret_cast<ObjHeader*>(header_ + 1); }
...
public:
    ArrayContainer(MemoryState* state, const TypeInfo* type_info, uint32_t elements) { Init(state, type_info, elements); }

    // Array container shalln't have any dtor, as it's being freed by ::Release().

    ArrayHeader* GetPlace() const { return reinterpret_cast<ArrayHeader*>(header_ + 1); }
...
struct ContainerChunk {
    ...
    ContainerHeader* asHeader() { return reinterpret_cast<ContainerHeader*>(this + 1); }
};
...
inline ContainerHeader* clearRemoved(ContainerHeader* container) {
    return reinterpret_cast<ContainerHeader*>(reinterpret_cast<uintptr_t>(container) & ~static_cast<uintptr_t>(1));
}
...
ContainerHeader* allocContainer(MemoryState* state, size_t size) {
    ...
    while (container != nullptr) {
        ...
        if (container->hasContainerSize() && container->containerSize() >= size && container->containerSize() <= size + 16) {
    ...
    if (result == nullptr) {
#if USE_GC
        if (state != nullptr) state->allocSinceLastGc += size;
#endif
...
void scheduleDestroyContainer(MemoryState* state, ContainerHeader* container) {
    ...
    if (!state->gcInProgress && state->finalizerQueueSuspendCount == 0 && state->finalizerQueueSize >= kFinalizerQueueThreshold) {
...
void freeContainer(ContainerHeader* container) {
    ...
    traverseContainerObjectFields(container, [container](ObjHeader** location) { ZeroHeapRef(location); });

    // And release underlying memory.
    if (isFreeable(container)) {
        container->setColorEvenIfGreen(CONTAINER_TAG_GC_BLACK);
        if (!container->buffered()) scheduleDestroyContainer(memoryState, container);
    }
}
 ...
 */
void depthFirstTraversal(ContainerHeader* start, bool* hasCycles, KRef* firstBlocker, KStdVector<ContainerHeader*>* order) {
    ContainerHeaderDeque toVisit;
    toVisit.push_back(start);
    start->setSeen();
    ...
    while (!toVisit.empty()) {
        ...
        traverseContainerReferredObjects(container, [container, hasCycles, firstBlocker, &order, &toVisit](ObjHeader* obj) {
            if (*firstBlocker != nullptr) return;
...
void dumpObject(ObjHeader* ref, int indent) {
    ...
    auto* packageName = typeInfo->packageName_ != nullptr ? CreateCStringFromString(typeInfo->packageName_) : nullptr;
    auto* relativeName = typeInfo->relativeName_ != nullptr ? CreateCStringFromString(typeInfo->relativeName_) : nullptr;
    MEMORY_LOG("%p %s.%s\n", ref, packageName ? packageName : "<unknown>", relativeName ? relativeName : "<unknown>");
...
void dumpContainerContent(ContainerHeader* container) {
    ...
    if (isAggregatingFrozenContainer(container)) {
        MEMORY_LOG(
                "%s aggregating container %p with %d objects rc=%d\n", colorNames[container->color()], container, container->objectCount(),
                container->refCount());
        ContainerHeader** subContainer = reinterpret_cast<ContainerHeader**>(container + 1);
        for (int i = 0; i < container->objectCount(); ++i) {
        ...
        MEMORY_LOG(
                "%s regular %s%scontainer %p with %d objects rc=%d\n", colorNames[container->color()], container->frozen() ? "frozen " : "",
                container->stack() ? "stack " : "", container, container->objectCount(), container->refCount());
        ObjHeader* obj = reinterpret_cast<ObjHeader*>(container + 1);
        dumpObject(obj, 4);
    }
}
        ...
        traverseContainerReferredObjects(container, [&toVisit](ObjHeader* ref) {
            ...
            if (!isShareable(childContainer)) {
                ...
                if (useColor) {
                    ...
                    if (color != CONTAINER_TAG_GC_BLACK) toVisit.push_front(childContainer);
                } else {
                    if (childContainer->marked()) toVisit.push_front(childContainer);
                }
            }
        });
    }
...
void markRoots(MemoryState* state) {
    for (auto container : *(state->toFree)) {
        if (isMarkedAsRemoved(container)) continue;
...
inline void addHeapRef(const ObjHeader* header) {
    ...
    if (container != nullptr) addHeapRef(const_cast<ContainerHeader*>(container));
}
...
inline void releaseHeapRef(const ObjHeader* header) {
    auto* container = header->container();
    if (container != nullptr) releaseHeapRef<Strict>(const_cast<ContainerHeader*>(container));
}
...
void processDecrements(MemoryState* state) {
    ...
    while (toRelease->size() > 0) {
        ...
        toRelease->pop_back();
        if (isMarkedAsRemoved(container)) continue;
        if (container->shareable()) container = realShareableContainer(container);
        decrementRC(container);
    }
    ...
    while (frame != nullptr) {
        ...
        while (current < end) {
            ...
            if (obj != nullptr) {
                ...
                auto* container = obj->container();
                if (container != nullptr) enqueueDecrementRC</* CanCollect = */ false>(container);
            }
        }
        frame = frame->previous;
    }
...
void garbageCollect(MemoryState* state, bool force) {
    ...
    if (g_hasCyclicCollector) cyclicLocalGC();
#endif // USE_CYCLIC_GC
...
void deinitInstanceBody(const TypeInfo* typeInfo, void* body) {
    for (int index = 0; index < typeInfo->objOffsetsCount_; index++) {
        ObjHeader** location = reinterpret_cast<ObjHeader**>(reinterpret_cast<uintptr_t>(body) + typeInfo->objOffsets_[index]);
        ZeroHeapRef(location);
    }
}
...
MemoryState* initMemory() {
    RuntimeAssert(offsetof(ArrayHeader, typeInfoOrMeta_) == offsetof(ObjHeader, typeInfoOrMeta_), "Layout mismatch");
    RuntimeAssert(offsetof(TypeInfo, typeInfo_) == offsetof(MetaObjHeader, typeInfo_), "Layout mismatch");
...
void setStackRef(ObjHeader** location, const ObjHeader* object) {
    ...
    if (!Strict && object != nullptr) addHeapRef(object);
...
void setHeapRef(ObjHeader** location, const ObjHeader* object) {
    ...
    if (object != nullptr) addHeapRef(const_cast<ObjHeader*>(object));
    *const_cast<const ObjHeader**>(location) = object;
}
...
OBJ_GETTER(initSharedInstance, ObjHeader** location, ObjHeader** localLocation, const TypeInfo* typeInfo, void (*ctor)(ObjHeader*)) {
    ...
    try {
        ctor(object);
        if (Strict) FreezeSubgraph(object);
        return object;
    } catch (...) {
        UpdateReturnRef(OBJ_RESULT, nullptr);
        ZeroHeapRef(location);
...
OBJ_GETTER(swapHeapRefLocked, ObjHeader** location, ObjHeader* expectedValue, ObjHeader* newValue, int32_t* spinlock, int32_t* cookie) {
    lock(spinlock);
...
#if USE_CYCLIC_GC
        if (g_hasCyclicCollector) cyclicMutateAtomicRoot(newValue);
#endif // USE_CYCLIC_GC
...
OBJ_GETTER(readHeapRefNoLock, ObjHeader* object, KInt index) {
    ...
    ObjHeader** location = reinterpret_cast<ObjHeader**>(reinterpret_cast<uintptr_t>(object) + object->type_info()->objOffsets_[index]);
    ObjHeader* value = *location;
#if USE_GC
...
void resumeGC() {
    ...
    if (state->gcSuspendCount > 0) {
        state->gcSuspendCount--;
        if (state->toRelease != nullptr && state->toRelease->size() >= state->gcThreshold && state->gcSuspendCount == 0) {
            garbageCollect(state, false);
        }
    }
}
...
void freezeAcyclic(ContainerHeader* rootContainer, ContainerHeaderSet* newlyFrozen) {
    ...
    while (!queue.empty()) {
        ...
        if (!current->frozen()) newlyFrozen->insert(current);
        MEMORY_LOG("freezing %p\n", current)
        current->freeze();
        traverseContainerReferredObjects(current, [current, &queue](ObjHeader* obj) {
            ContainerHeader* objContainer = obj->container();
            if (canFreeze(objContainer)) {
                if (objContainer->marked()) queue.push_back(objContainer);
            }
        });
    }
}

void freezeCyclic(ObjHeader* root, const KStdVector<ContainerHeader*>& order, ContainerHeaderSet* newlyFrozen) {
    ...
    while (!queue.empty()) {
        ...
        traverseContainerReferredObjects(currentContainer, [current, currentContainer, &queue, &reversedEdges](ObjHeader* obj) {
            ContainerHeader* objContainer = obj->container();
            if (canFreeze(objContainer)) {
                if (objContainer->marked()) queue.push_back(obj);
                // We ignore references from FreezableAtomicsReference during condensation, to avoid KT-33824.
                if (!isFreezableAtomic(current))
                    reversedEdges.emplace(objContainer, KStdVector<ContainerHeader*>(0)).first->second.push_back(currentContainer);
            }
        });
    }
    ...
    for (auto it = order.rbegin(); it != order.rend(); ++it) {
        ...
        MEMORY_LOG("SCC:\n");
#if TRACE_MEMORY
        for (auto c : component) konan::consolePrintf("    %p\n", c);
#endif
    ...
    for (auto it = components.rbegin(); it != components.rend(); ++it) {
        ...
        for (auto* container : component) {
            ...
            traverseContainerReferredObjects(container, [&internalRefsCount](ObjHeader* obj) {
                auto* container = obj->container();
                if (canFreeze(container)) ++internalRefsCount;
            });
        }
        ...
        for (auto* container : component) {
            ...
            if (!container->frozen()) newlyFrozen->insert(container);
            // Note, that once object is frozen, it could be concurrently accessed, so
            // color and similar attributes shall not be used.
            MEMORY_LOG("freezing %p\n", container)
            container->freeze();
...
void freezeSubgraph(ObjHeader* root) {
    ...
    KRef firstBlocker = root->has_meta_object() && ((root->meta_object()->flags_ & MF_NEVER_FROZEN) != 0) ? root : nullptr;
...
void ensureNeverFrozen(ObjHeader* object) {
    ...
    if (container == nullptr || container->frozen()) ThrowFreezingException(object, object);
...
void ArrayContainer::Init(MemoryState* state, const TypeInfo* typeInfo, uint32_t elements) {
    ...
    uint32_t allocSize = sizeof(ContainerHeader) + arrayObjectSize(typeInfo, elements);
    header_ = allocContainer(state, allocSize);
    RuntimeCheck(header_ != nullptr, "Cannot alloc memory");
    // One object in this container, no need to set.
    header_->setContainerSize(allocSize);
...
#if USE_GC
    if (IsStrictMemoryModel && object != nullptr && isShareable(object->container())) rememberNewContainer(object->container());
#endif // USE_GC
}
...
OBJ_GETTER(InitInstanceStrict, ObjHeader** location, const TypeInfo* typeInfo, void (*ctor)(ObjHeader*)) {
...
#if USE_CYCLIC_GC
    if (g_hasCyclicCollector) cyclicScheduleGarbageCollect();
#else
...
#if USE_CYCLIC_GC
    if (g_hasCyclicCollector) cyclicCollectorCallback(worker);
#endif // USE_CYCLIC_GC
}
...
#else
    if (value) ThrowIllegalArgumentException();
#endif // USE_CYCLIC_GC
}
