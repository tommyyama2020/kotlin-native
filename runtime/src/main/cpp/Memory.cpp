            ...
            for (int j = 0; j < 6; j++) {
                if (updateCounters[i][j] != 0)
                    konan::consolePrintf(
                        ...
                        indexToName[j], updateCounters[i][j], percents(updateCounters[i][j], allUpdateRefs),
            ...
            for (int j = 6; j < 12; j++) {
                if (updateCounters[i][j] != 0)
                    konan::consolePrintf(
                        ...
                        indexToName[j - 6], updateCounters[i][j], percents(updateCounters[i][j], allUpdateRefs),
...
void depthFirstTraversal(
    ContainerHeader* start, bool* hasCycles, KRef* firstBlocker, KStdVector<ContainerHeader*>* order) {
    ContainerHeaderDeque toVisit;
    toVisit.push_back(start);
    start->setSeen();
    ...
    while (!toVisit.empty()) {
        ...
        traverseContainerReferredObjects(
            container, [container, hasCycles, firstBlocker, &order, &toVisit](ObjHeader* obj) {
                ...
                if (canFreeze(objContainer)) {
                    ...
                    if (!objContainer->seen() && !objContainer->marked()) {
                        ...
                        // Here we do rather interesting trick: when doing DFS we postpone processing references going
                        // from FreezableAtomic, so that in 'order' referred value will be seen as not actually
                        // belonging to the same SCC (unless there are other edges not going through FreezableAtomic
...
void dumpContainerContent(ContainerHeader* container) {
    ...
    if (isAggregatingFrozenContainer(container)) {
        ...
        MEMORY_LOG(
            ...
            container->frozen() ? "frozen " : "", container->stack() ? "stack " : "", container,
            container->objectCount(), container->refCount());
        ObjHeader* obj = reinterpret_cast<ObjHeader*>(container + 1);
        dumpObject(obj, 4);
    }
...
void garbageCollect(MemoryState* state, bool force) {
    ...
    GC_LOG(
        ">>> %s GC: threshold = %d toFree %d toRelease %d alloc = %lld\n", force ? "forced" : "regular",
        state->gcThreshold, state->toFree->size(), state->toRelease->size(), allocSinceLastGc)
...
void deinitInstanceBody(const TypeInfo* typeInfo, void* body) {
    for (int index = 0; index < typeInfo->objOffsetsCount_; index++) {
        ObjHeader** location =
            reinterpret_cast<ObjHeader**>(reinterpret_cast<uintptr_t>(body) + typeInfo->objOffsets_[index]);
        ZeroHeapRef(location);
    }
}
...
OBJ_GETTER(readHeapRefNoLock, ObjHeader* object, KInt index) {
    ...
    ObjHeader** location =
        reinterpret_cast<ObjHeader**>(reinterpret_cast<uintptr_t>(object) + object->type_info()->objOffsets_[index]);
    ObjHeader* value = *location;
#if USE_GC
...
void resumeGC() {
    ...
    if (state->gcSuspendCount > 0) {
        state->gcSuspendCount--;
        if (state->toRelease != nullptr && state->toRelease->size() >= state->gcThreshold &&
...
void freezeCyclic(ObjHeader* root, const KStdVector<ContainerHeader*>& order, ContainerHeaderSet* newlyFrozen) {
    ...
    while (!queue.empty()) {
        ...
        traverseContainerReferredObjects(
            currentContainer, [current, currentContainer, &queue, &reversedEdges](ObjHeader* obj) {
                ContainerHeader* objContainer = obj->container();
                if (canFreeze(objContainer)) {
                    if (objContainer->marked())
                        queue.push_back(obj);
    ...
    for (auto it = components.rbegin(); it != components.rend(); ++it) {
        ...
        MEMORY_LOG(
            "Setting aggregating %p rc to %d (total %d inner %d)\n", superContainer, totalCount - internalRefsCount,
            totalCount, internalRefsCount)
...
void freezeSubgraph(ObjHeader* root) {
    ...
    KRef firstBlocker =
        root->has_meta_object() && ((root->meta_object()->flags_ & MF_NEVER_FROZEN) != 0) ? root : nullptr;
