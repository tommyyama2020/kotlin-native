...
// PLEASE READ: please do not alter signatures of the existing functions, and
// when adding the new function please do not forget to add new functions into
// the debug operations list.
...
RUNTIME_USED RUNTIME_WEAK int32_t
Konan_DebugObjectToUtf8Array(KRef obj, char* buffer, int32_t bufferSize);
...
RUNTIME_USED RUNTIME_WEAK int32_t Konan_DebugGetFieldType(KRef obj,
                                                          int32_t index);

// Compute address of field or an array element at the index, or null, if
// incorrect.
RUNTIME_USED RUNTIME_WEAK void* Konan_DebugGetFieldAddress(KRef obj,
                                                           int32_t index);

// Compute address of field or an array element at the index, or null, if
// incorrect.
RUNTIME_USED RUNTIME_WEAK const char* Konan_DebugGetFieldName(KRef obj,
                                                              int32_t index);
...
/**
 * Given an object finds debugger interface operation suitable for manipulation
 * with this object. Important for cases where multiple K/N runtimes coexist in
 * the same address space and debugger doesn't know which debug operation to use
 * on particular instance.
 */
RUNTIME_USED RUNTIME_WEAK void* Konan_DebugGetOperation(
    KRef obj, /* Konan_DebugOperation */ int32_t operation);
