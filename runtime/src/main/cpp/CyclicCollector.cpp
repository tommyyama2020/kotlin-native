    ...
    void gcProcessor() {
        {
            ...
            restart:
                ...
                if (toRelease_.size() > 0) atomicSet(&pendingRelease_, 1);
...
#if WITH_WORKERS
    auto* local = cyclicCollector;
    if (local) local->addWorker(worker);
#endif // WITH_WORKERS
}
...
#if WITH_WORKERS
    auto* local = cyclicCollector;
    if (local) local->removeWorker(worker, enabled);
#endif // WITH_WORKERS
}
...
#if WITH_WORKERS
    auto* local = cyclicCollector;
    if (local) local->collectorCallaback(worker);
#endif // WITH_WORKERS
}
...
#if WITH_WORKERS
    auto* local = cyclicCollector;
    if (local) local->scheduleGarbageCollect();
#endif // WITH_WORKERS
}
...
#if WITH_WORKERS
    auto* local = cyclicCollector;
    if (local) local->mutateRoot(newValue);
#endif // WITH_WORKERS
}
