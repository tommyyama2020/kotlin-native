...
KInt Kotlin_AtomicInt_compareAndSwap(KRef thiz, KInt expectedValue,
                                     KInt newValue) {
  return compareAndSwapImpl(thiz, expectedValue, newValue);
}

KBoolean Kotlin_AtomicInt_compareAndSet(KRef thiz, KInt expectedValue,
                                        KInt newValue) {
...
KLong Kotlin_AtomicLong_compareAndSwap(KRef thiz, KLong expectedValue,
                                       KLong newValue) {
#if KONAN_NO_64BIT_ATOMIC
...
KBoolean Kotlin_AtomicLong_compareAndSet(KRef thiz, KLong expectedValue,
                                         KLong newValue) {
#if KONAN_NO_64BIT_ATOMIC
  // Potentially huge performance penalty, but correct.
...
KNativePtr Kotlin_AtomicNativePtr_compareAndSwap(KRef thiz,
                                                 KNativePtr expectedValue,
                                                 KNativePtr newValue) {
  return compareAndSwapImpl(thiz, expectedValue, newValue);
}

KBoolean Kotlin_AtomicNativePtr_compareAndSet(KRef thiz,
                                              KNativePtr expectedValue,
                                              KNativePtr newValue) {
  return compareAndSetImpl(thiz, expectedValue, newValue);
}
...
OBJ_GETTER(Kotlin_AtomicReference_compareAndSwap, KRef thiz, KRef expectedValue,
           KRef newValue) {
...
KBoolean Kotlin_AtomicReference_compareAndSet(KRef thiz, KRef expectedValue,
                                              KRef newValue) {
  Kotlin_AtomicReference_checkIfFrozen(newValue);
  // See Kotlin_AtomicReference_get() for explanations, why locking is needed.
  AtomicReferenceLayout* ref = asAtomicReference(thiz);
...
OBJ_GETTER(Kotlin_AtomicReference_get, KRef thiz) {
  // Here we must take a lock to prevent race when value, while taken here, is
  // CASed and immediately destroyed by an another thread. AtomicReference no
  // longer holds such an object, so if we got rescheduled unluckily, between
  // the moment value is read from the field and RC is incremented, object may
  // go away.
  AtomicReferenceLayout* ref = asAtomicReference(thiz);
