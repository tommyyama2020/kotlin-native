...
void initSymbols() {
  ...
  if (ehdr == nullptr)
    return;
  RuntimeAssert(strncmp((const char*)ehdr->e_ident, ELFMAG, SELFMAG) == 0,
                "Must be an ELF");
  ...
  for (int i = 0; i < ehdr->e_shnum; i++) {
    if (shdr[i].sh_type == SHT_SYMTAB) {  // Static symbol table.
      ...
      record.symtabEnd =
          (Elf_Sym*)((char*)record.symtabBegin + shdr[i].sh_size);
...
const char* addressToSymbol(const void* address) {
  ...
  for (auto record : *symbols) {
    ...
    while (begin < end) {
      ...
      if (addressValue >= begin->st_value &&
          addressValue < begin->st_value + begin->st_size) {
...
static void* mapModuleFile(HMODULE hModule) {
  ...
  LPVOID mapAddress = MapViewOfFile(hFileMappingObject,
...
class SymbolTable {
 private:
  ...
  void getSymbolName(IMAGE_SYMBOL* sym, char* resultBuffer,
                     size_t resultBufferSize) {
    if (sym->N.Name.Short != 0) {
      // ShortName is not zero-terminated if its length exactly equals
      // SYMBOL_SHORT_NAME_LENGTH. Copy it to the buffer and zero-terminate
      // explicitly:
      size_t bytesToCopy = SYMBOL_SHORT_NAME_LENGTH;
      if (bytesToCopy > resultBufferSize - 1)
        bytesToCopy = resultBufferSize - 1;
  ...
  const void* getSymbolAddress(IMAGE_SYMBOL* symbol) {
    IMAGE_SECTION_HEADER* sectionHeader =
        &sectionHeaders[symbol->SectionNumber - 1];
    return (const void*)(imageBase + sectionHeader->VirtualAddress +
                         symbol->Value);
  }
 ...
 public:
  explicit SymbolTable(HMODULE hModule) {
    ...
    RuntimeAssert(dosHeader->e_magic == IMAGE_DOS_SIGNATURE,
                  "PE executable e_magic mismatch");

    IMAGE_NT_HEADERS* ntHeaders =
        (IMAGE_NT_HEADERS*)(imageBase + dosHeader->e_lfanew);
    RuntimeAssert(ntHeaders->Signature == IMAGE_NT_SIGNATURE,
                  "PE executable NT signature mismatch");

    IMAGE_FILE_HEADER* fileHeader = &ntHeaders->FileHeader;

    sectionHeaders = (IMAGE_SECTION_HEADER*)(((char*)(fileHeader + 1)) +
    ...
    // Symbol table doesn't get mapped to the memory, so we have to load it
    // ourselves:
    char* mappedModuleFile = (char*)mapModuleFile(hModule);
    if (mappedModuleFile != nullptr) {
      symbols =
          (IMAGE_SYMBOL*)(mappedModuleFile + fileHeader->PointerToSymbolTable);
...
extern "C" bool AddressToSymbol(const void* address, char* resultBuffer,
  ...
  if (theExeSymbolTable == nullptr) {
    // Note: do not protecting the lazy initialization by critical sections for
    // simplicity; this doesn't have any serious consequences.
    HMODULE hModule = nullptr;
    int rv = GetModuleHandleExW(
  ...
  return theExeSymbolTable->functionAddressToSymbol(address, resultBuffer,
                                                    resultBufferSize);
}
