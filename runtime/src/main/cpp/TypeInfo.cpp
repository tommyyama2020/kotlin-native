...
// Seeks for the specified id. In case of failure returns a valid pointer to
// some record, never returns nullptr. It is the caller's responsibility to
// check if the search has succeeded or not.
InterfaceTableRecord const* LookupInterfaceTableRecord(
  ...
  if (interfaceTableSize <= 8) {
    ...
    int i;
    for (i = 0;
         i < interfaceTableSize - 1 && interfaceTable[i].id < interfaceId; ++i)
      ;
