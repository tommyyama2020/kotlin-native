...
void consoleInit() {
#if KONAN_WINDOWS
  // Note that this code enforces UTF-8 console output, so we may want to
  // rethink how we perform console IO, if it turns out, that UTF-16 is better
  // output format.
...
static void onThreadExitInit() {
  ...
  // As one may see, glibc checks value of the key even if it was not inited
  // (and == 0), and so data associated with our legit key (== 0 as being the
  // first one) is used. Other libc are not affected, as usually == 0 pthread
  // key is impossible.
...
#error "How to do onThreadExit()?"
  ...
  DestructorRecord* destructorRecord =
      (DestructorRecord*)calloc(1, sizeof(DestructorRecord));
...
// memcpy/memmove are not here intentionally, as frequently
...
using steady_time_clock =
    std::conditional<high_resolution_clock::is_steady, high_resolution_clock,
                     steady_clock>::type;

uint64_t getTimeMillis() {
  return duration_cast<milliseconds>(
             steady_time_clock::now().time_since_epoch())
      .count();
}

uint64_t getTimeNanos() {
  return duration_cast<nanoseconds>(steady_time_clock::now().time_since_epoch())
      .count();
}
