...
static void writeStackTraceToBuffer(KRef throwable, char* buffer,
                                    unsigned long bufferSize) {
  if (bufferSize < 2)
    return;

  ObjHolder stackTraceHolder;
  ArrayHeader* stackTrace =
      Kotlin_Throwable_getStackTrace(throwable, stackTraceHolder.slot())
          ->array();
  ...
  *(bufferPointer++) = '(';
  ...
  for (int index = 0; index < stackTrace->count_; ++index) {
    KNativePtr ptr =
        *PrimitiveArrayAddressOfElementAt<KNativePtr>(stackTrace, index);
    int bytes = snprintf(bufferPointer, remainingBytes, "0x%" PRIxPTR " ",
...
static char* findExceptionBacktraceSection(unsigned long* size) {
  ...
  for (uint32_t loadCommandIndex = 0; loadCommandIndex < header->ncmds;
       ++loadCommandIndex) {
    if (loadCommand->cmd == LC_SEGMENT_TARGET) {
      segment_command_target* segmentCommand =
          reinterpret_cast<segment_command_target*>(loadCommand);
      ...
      if (bufferEqualsString(segmentCommand->segname, "__DATA")) {
        section_target* sections =
            reinterpret_cast<section_target*>(segmentCommand + 1);
        for (uint32_t sectionIndex = 0; sectionIndex < segmentCommand->nsects;
             ++sectionIndex) {
    ...
    loadCommand = reinterpret_cast<load_command*>(
        reinterpret_cast<uintptr_t>(loadCommand) + loadCommand->cmdsize);
  }
...
void ReportBacktraceToIosCrashLog(KRef throwable) {
  ...
  // Note: access to this buffer is protected by a lock, but it is not easily
  // accessible. Instead assume that typically this buffer is accessed only
  // during termination, and rely on caller guaranteeing this code to be
  // executed only before system termination handlers.
