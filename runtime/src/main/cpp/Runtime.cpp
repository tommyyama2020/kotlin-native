...
enum { INIT_GLOBALS = 0, INIT_THREAD_LOCAL_GLOBALS = 1, DEINIT_THREAD_LOCAL_GLOBALS = 2, DEINIT_GLOBALS = 3 };

enum { SUSPENDED = 0, RUNNING, DESTROYING };
...
void deinitRuntime(RuntimeState* state) {
    ...
    if (lastRuntime) InitOrDeinitGlobalVariables(DEINIT_GLOBALS, state->memoryState);
...
void CheckIsMainThread() {
    if (!isMainThread) ThrowIncorrectDereferenceException();
}
