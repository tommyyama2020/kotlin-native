...
static void setAssociatedTypeInfo(Class clazz, const TypeInfo* typeInfo) {
    objc_setAssociatedObject(
        clazz, &associatedTypeInfoKey, [NSValue valueWithPointer:typeInfo], OBJC_ASSOCIATION_RETAIN);
}
...
extern "C" OBJ_GETTER(
    Kotlin_ObjCExport_AllocInstanceWithAssociatedObject, const TypeInfo* typeInfo, id associatedObject) RUNTIME_NOTHROW;

extern "C" OBJ_GETTER(
    Kotlin_ObjCExport_AllocInstanceWithAssociatedObject, const TypeInfo* typeInfo, id associatedObject) {
...
static const ObjCTypeAdapter* findClassAdapter(Class clazz) {
    return findAdapterByName(
        class_getName(clazz), Kotlin_ObjCExport_sortedClassAdapters, Kotlin_ObjCExport_sortedClassAdaptersNum);
}

static const ObjCTypeAdapter* findProtocolAdapter(Protocol* prot) {
    return findAdapterByName(
        protocol_getName(prot), Kotlin_ObjCExport_sortedProtocolAdapters, Kotlin_ObjCExport_sortedProtocolAdaptersNum);
}
...
static void Kotlin_ObjCExport_initializeImpl() {
    ...
    for (const char* swiftRootClassName : {"SwiftObject", "_TtCs12_SwiftObject"}) {
        ...
        if (swiftRootClass != nullptr) {
            added =
                class_addMethod(swiftRootClass, toKotlinSelector, (IMP)SwiftObject_toKotlinImp, toKotlinTypeEncoding);
...
static convertReferenceToObjC findConverterFromInterfaces(const TypeInfo* typeInfo) {
    ...
    return foundTypeInfo == nullptr ? nullptr
...
static void buildITable(TypeInfo* result, const KStdOrderedMap<ClassId, KStdVector<VTableElement>>& interfaceVTables) {
    ...
    for (int i = 0; i < itableSize; ++i) {
        ...
        auto interfaceVTable_ =
            interfaceVTableSize == 0 ? nullptr : konanAllocArray<VTableElement>(interfaceVTableSize);
...
static void insertOrReplace(
    KStdVector<MethodTableRecord>& methodTable, MethodNameHash nameSignature, void* entryPoint) {
...
static void throwIfCantBeOverridden(Class clazz, const KotlinToObjCMethodAdapter* adapter) {
    if (adapter->kotlinImpl == nullptr) {
        ...
        [NSException raise:NSGenericException
                    format:@"[%s %s] can't be overridden: %@", class_getName(clazz), adapter->selector, reason];
    }
}
...
static const TypeInfo* createTypeInfo(Class clazz, const TypeInfo* superType) {
    ...
    const TypeInfo* result = createTypeInfo(
        superType, addedInterfaces, vtable, methodTable, interfaceVTables, superITable, superITableSize,
        itableEqualsSuper);
...
extern "C" void Kotlin_ObjCExport_AbstractMethodCalled(id self, SEL selector) {
    [NSException raise:NSGenericException
                format:@"[%s %s] is abstract", class_getName(object_getClass(self)), sel_getName(selector)];
}
