...
extern "C" OBJ_GETTER(Kotlin_ObjCExport_AllocInstanceWithAssociatedObject, const TypeInfo* typeInfo, id associatedObject) RUNTIME_NOTHROW;

extern "C" OBJ_GETTER(Kotlin_ObjCExport_AllocInstanceWithAssociatedObject, const TypeInfo* typeInfo, id associatedObject) {
...
extern "C" id Kotlin_ObjCExport_CreateNSStringFromKString(ObjHeader* str) {
    ...
    if (str->permanent()) {
        return [[[NSString alloc] initWithBytesNoCopy:utf16Chars length:numBytes encoding:NSUTF16LittleEndianStringEncoding
                                         freeWhenDone:NO] autorelease];
    } else {
        // TODO: consider making NSString subclass to avoid copying here.
        NSString* candidate = [[NSString alloc] initWithBytes:utf16Chars length:numBytes encoding:NSUTF16LittleEndianStringEncoding];
...
static const ObjCTypeAdapter* findAdapterByName(const char* name, const ObjCTypeAdapter** sortedAdapters, int adapterNum) {
...
static const ObjCTypeAdapter* findClassAdapter(Class clazz) {
    return findAdapterByName(class_getName(clazz), Kotlin_ObjCExport_sortedClassAdapters, Kotlin_ObjCExport_sortedClassAdaptersNum);
}

static const ObjCTypeAdapter* findProtocolAdapter(Protocol* prot) {
    return findAdapterByName(protocol_getName(prot), Kotlin_ObjCExport_sortedProtocolAdapters, Kotlin_ObjCExport_sortedProtocolAdaptersNum);
}
    ...
    for (const char* swiftRootClassName : {"SwiftObject", "_TtCs12_SwiftObject"}) {
        ...
        if (swiftRootClass != nullptr) {
            ...
            added = class_addMethod(
                    swiftRootClass, releaseAsAssociatedObjectSelector, (IMP)SwiftObject_releaseAsAssociatedObjectImp,
...
static const char* getBlockEncoding(id block) {
    ...
    return (flags & (1 << 25)) != 0 ? literal->descriptor->signature
...
static OBJ_GETTER(blockToKotlinImp, id block, SEL cmd) {
    ...
    for (int i = 1; i <= parameterCount; ++i) {
        ...
        if (argEncoding[0] != '@') {
            [NSException raise:NSGenericException format:@"Blocks with non-reference-typed arguments aren't supported (%s)", argEncoding];
        }
    }
...
static convertReferenceToObjC findConverterFromInterfaces(const TypeInfo* typeInfo) {
    ...
    return foundTypeInfo == nullptr ? nullptr : (convertReferenceToObjC)foundTypeInfo->writableInfo_->objCExport.convert;
}
...
static void throwIfCantBeOverridden(Class clazz, const KotlinToObjCMethodAdapter* adapter) {
    if (adapter->kotlinImpl == nullptr) {
        ...
        [NSException raise:NSGenericException format:@"[%s %s] can't be overridden: %@", class_getName(clazz), adapter->selector, reason];
    }
}
...
static const TypeInfo* createTypeInfo(Class clazz, const TypeInfo* superType) {
    ...
    KStdVector<MethodTableRecord> methodTable(superMethodTable, superMethodTable + superMethodTableSize);
    ...
    for (const TypeInfo* t : supers) {
        ...
        for (int i = 0; i < typeAdapter->reverseAdapterNum; ++i) {
            ...
            if (definedSelectors.find(sel_registerName(adapter->selector)) == definedSelectors.end())
                continue;
    ...
    const TypeInfo* result = createTypeInfo(
            superType, addedInterfaces, vtable, methodTable, interfaceVTables, superITable, superITableSize, itableEqualsSuper);
...
static const TypeInfo* getOrCreateTypeInfo(Class clazz) {
    ...
    const TypeInfo* superType = superClass == nullptr ? theForeignObjCObjectTypeInfo : getOrCreateTypeInfo(superClass);
...
extern "C" void Kotlin_ObjCExport_AbstractMethodCalled(id self, SEL selector) {
    [NSException raise:NSGenericException format:@"[%s %s] is abstract", class_getName(object_getClass(self)), sel_getName(selector)];
}
