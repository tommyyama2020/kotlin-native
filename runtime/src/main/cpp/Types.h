...
typedef std::basic_string<char, std::char_traits<char>, KonanAllocator<char>>
    KStdString;
...
using KStdOrderedMap =
    std::map<Key, Value, Compare, KonanAllocator<std::pair<const Key, Value>>>;
template <class Value>
...
KBoolean IsInstance(const ObjHeader* obj,
                    const TypeInfo* type_info) RUNTIME_PURE;
