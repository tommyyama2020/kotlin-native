...
struct ContainerHeader {
...
#else
        int value =
            Atomic ? __sync_sub_and_fetch(&refCount_, CONTAINER_TAG_INCREMENT) : refCount_ -= CONTAINER_TAG_INCREMENT;
#endif
    ...
    inline int decRefCount() {
        ...
        int value = refCount_ -= CONTAINER_TAG_INCREMENT;
#else
        int value = shareable() ? __sync_sub_and_fetch(&refCount_, CONTAINER_TAG_INCREMENT)
                                : refCount_ -= CONTAINER_TAG_INCREMENT;
#endif
        return value >> CONTAINER_TAG_SHIFT;
    }
