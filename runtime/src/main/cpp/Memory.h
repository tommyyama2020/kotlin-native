...
struct ContainerHeader {
    ...
    inline bool local() const { return (refCount_ & CONTAINER_TAG_MASK) == CONTAINER_TAG_LOCAL; }

    inline bool frozen() const { return (refCount_ & CONTAINER_TAG_MASK) == CONTAINER_TAG_FROZEN; }

    inline void freeze() { refCount_ = (refCount_ & ~CONTAINER_TAG_MASK) | CONTAINER_TAG_FROZEN; }

    inline void makeShared() { refCount_ = (refCount_ & ~CONTAINER_TAG_MASK) | CONTAINER_TAG_SHARED; }

    inline bool shared() const { return (refCount_ & CONTAINER_TAG_MASK) == CONTAINER_TAG_SHARED; }

    inline bool shareable() const {
        return (tag() & 1) != 0; // CONTAINER_TAG_FROZEN || CONTAINER_TAG_SHARED
    }

    inline bool stack() const { return (refCount_ & CONTAINER_TAG_MASK) == CONTAINER_TAG_STACK; }
...
#else
        int value = Atomic ? __sync_sub_and_fetch(&refCount_, CONTAINER_TAG_INCREMENT) : refCount_ -= CONTAINER_TAG_INCREMENT;
#endif
    ...
    inline int decRefCount() {
        ...
        int value = refCount_ -= CONTAINER_TAG_INCREMENT;
#else
        int value = shareable() ? __sync_sub_and_fetch(&refCount_, CONTAINER_TAG_INCREMENT) : refCount_ -= CONTAINER_TAG_INCREMENT;
#endif
        return value >> CONTAINER_TAG_SHIFT;
    }
    ...
    inline unsigned objectCount() const {
        return (objectCount_ & CONTAINER_TAG_GC_HAS_OBJECT_COUNT) != 0 ? (objectCount_ >> CONTAINER_TAG_GC_SHIFT) : 1;
    }
    ...
    inline bool hasContainerSize() { return (objectCount_ & CONTAINER_TAG_GC_HAS_OBJECT_COUNT) == 0; }

    inline unsigned color() const { return objectCount_ & CONTAINER_TAG_GC_COLOR_MASK; }
    ...
    inline bool buffered() const { return (objectCount_ & CONTAINER_TAG_GC_BUFFERED) != 0; }
    ...
    inline bool marked() const { return (objectCount_ & CONTAINER_TAG_GC_MARKED) != 0; }

    inline void mark() { objectCount_ |= CONTAINER_TAG_GC_MARKED; }

    inline void unMark() { objectCount_ &= ~CONTAINER_TAG_GC_MARKED; }

    inline bool seen() const { return (objectCount_ & CONTAINER_TAG_GC_SEEN) != 0; }
    ...
    inline void setNextLink(ContainerHeader* next) { *reinterpret_cast<ContainerHeader**>(this + 1) = next; }

    inline ContainerHeader* nextLink() { return *reinterpret_cast<ContainerHeader**>(this + 1); }
};
...
struct ObjHeader {
    ...
    const TypeInfo* type_info() const { return clearPointerBits(typeInfoOrMeta_, OBJECT_TAG_MASK)->typeInfo_; }
    ...
    MetaObjHeader* meta_object() {
        return has_meta_object() ? reinterpret_cast<MetaObjHeader*>(clearPointerBits(typeInfoOrMeta_, OBJECT_TAG_MASK))
    ...
    inline bool permanent() const { return hasPointerBits(typeInfoOrMeta_, OBJECT_TAG_PERMANENT_CONTAINER); }
...
void SetHeapRefLocked(ObjHeader** location, ObjHeader* newValue, int32_t* spinlock, int32_t* cookie) RUNTIME_NOTHROW;
...
class ObjHolder {
public:
    ObjHolder() : obj_(nullptr) { EnterFrame(frame(), 0, sizeof(*this) / sizeof(void*)); }
...
public:
    explicit ExceptionObjHolder(const ObjHeader* obj) { ::SetHeapRef(&obj_, obj); }
