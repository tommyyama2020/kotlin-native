...
extern "C" void Kotlin_ObjCExport_runCompletionFailure(KRef completionHolder, KRef exception, const TypeInfo** exceptionTypes) {
    id error = Kotlin_ObjCExport_ExceptionAsNSError(exception, exceptionTypes);
    Completion completion = (Completion)GetAssociatedObject(completionHolder);
    completion(nullptr, error);
}

extern "C" OBJ_GETTER(Kotlin_ObjCExport_createContinuationArgumentImpl, KRef completionHolder, const TypeInfo** exceptionTypes);
...
extern "C" OBJ_GETTER(Kotlin_ObjCExport_createContinuationArgument, id completion, const TypeInfo** exceptionTypes) {
    ...
    ObjHolder slot;
    KRef completionHolder = AllocInstanceWithAssociatedObject(theForeignObjCObjectTypeInfo, objc_retainBlock(completion), slot.slot());
...
extern "C" void Kotlin_ObjCExport_resumeContinuation(KRef continuation, id result, id error) {
    ...
    if (error != nullptr) {
        if (result != nullptr) {
            [NSException raise:NSGenericException
                        format:@"Kotlin completion handler is called with both result (%@) and error (%@) specified", result, error];
        }
