...
extern "C" void Kotlin_ObjCExport_runCompletionSuccess(KRef completionHolder,
                                                       KRef result) {
...
extern "C" void Kotlin_ObjCExport_runCompletionFailure(
    KRef completionHolder, KRef exception, const TypeInfo** exceptionTypes) {
...
extern "C" OBJ_GETTER(Kotlin_ObjCExport_createContinuationArgument,
                      id completion, const TypeInfo** exceptionTypes) {
  if (pthread_main_np() != 1) {
    [NSException raise:NSGenericException
                format:
                    @"Calling Kotlin suspend functions from Swift/Objective-C "
                    @"is currently supported only on main thread"];
  }
  ObjHolder slot;
  KRef completionHolder = AllocInstanceWithAssociatedObject(
      theForeignObjCObjectTypeInfo, objc_retainBlock(completion), slot.slot());

  RETURN_RESULT_OF(Kotlin_ObjCExport_createContinuationArgumentImpl,
                   completionHolder, exceptionTypes);
}

extern "C" void Kotlin_ObjCExport_resumeContinuationSuccess(KRef continuation,
                                                            KRef result);
extern "C" void Kotlin_ObjCExport_resumeContinuationFailure(KRef continuation,
                                                            KRef exception);

extern "C" void Kotlin_ObjCExport_resumeContinuation(KRef continuation,
                                                     id result, id error) {
  ObjHolder holder;
  ...
  if (error != nullptr) {
    if (result != nullptr) {
      [NSException raise:NSGenericException
                  format:
                      @"Kotlin completion handler is called with both result "
                      @"(%@) and error (%@) specified",
                      result, error];
    }
